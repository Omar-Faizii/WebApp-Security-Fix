<?php
$dbh = new mysqli("localhost", "root", "password", "ecommerce1") or die(mysqli_error($con));
session_start();
...
$pass=$password;
		$stmt = $dbh->prepare("insert into users(name,email,password,contact,city,address) values('$name','$email','$password','$contact','$city','$address')");
	    $result=$stmt->execute();
	if($result===true)
	{
